<template>
  <div class="app-container">
    ...
    <el-form ref="form" :inline="true" :model="form" size="small">
      <el-form-item label="热门词" prop="word">
        <el-input v-model="form.word" placeholder="请输入热门词" @keyup.enter.native="handleSearch" />
      </el-form-item>
      <el-form-item>
    ...
    >
      ...
      <el-table-column property="word" align="center" label="热门词" />
    ...
    <el-dialog :title="title" :visible.sync="dialogVisible">
      <div>
        <el-form ref="filter" :rules="filterRules" :model="filter" label-width="100px">
          ...
          <el-form-item label="热门词" prop="word">
            <el-input v-model.trim="filter.word" />
          </el-form-item>
...
<script>
import { articleWordList, dealArticleWord } from '@/api/member'
...
export default {
  ...
  data() {
    return {
      ...
      filterRules: {
        word: [{ required: true, message: '请输入热门词', trigger: 'blur' }]
      },
      list: [],
      total: 0,
      dialogVisible: false,
  ...
  methods: {
    ...
    handleCommit() {
      this.$refs.filter.validate((valid) => {
        ...
        dealArticleWord({ operateType, ...this.filter }).then(response => {
    ...
    handleDel(id) {
      this.$confirm(this.$t('table.confirmDelete'), this.$t('table.tip'), {
        ...
        dealArticleWord({ id, operateType: '3' }).then(response => {
          this.$message.success(this.$t('msg.success'))
          this.getList()
        })
      }).catch(() => {
    ...
    getList() {
      ...
      articleWordList(this.listQuery).then(response => {

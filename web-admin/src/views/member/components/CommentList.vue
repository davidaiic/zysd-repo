  ...
  >
    <div class="app-container">
      ...
      >
        ...
        <el-table-column property="created" align="center" label="评论时间" />
...
import { articleCommentList, dealArticleComment } from '@/api/member'
...
export default {
  ...
  created() {
    this.commentDialogVisible = this.isShow
    this.getList()
  },
  methods: {
    ...
    // 搜索
    getList() {
      this.listQuery = Object.assign(this.listQuery, this.form, { articleId: this.articleId })
      articleCommentList(this.listQuery).then(response => {
    ...
    handleChange(commentId, value, type) {
      dealArticleComment({ commentId, value, type }).then(response => {
    ...
    handleDel(commentId) {
      this.$confirm(this.$t('table.confirmDelete'), this.$t('table.tip'), {
        ...
        dealArticleComment({ commentId, value: 1, type: 2 }).then(response => {
          this.$message.success(this.$t('msg.success'))
          this.getList()
        })
      }).catch(() => {

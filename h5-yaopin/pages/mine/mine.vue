<template>
	<view class="mainWrap">
		<view class="contents">
			<u-navbar v-if="!isH5" class="searchBars" :fixed="false" leftIcon=" " placeholder>
			...
			<view class="content_avatar">
				<view class="content_avatar_icon">
					<view class="avatar_icon_inner">
						<u-image v-if="formData.avatar" :src="formData.avatar" width="60rpx" height="60rpx"></u-image>
						<u-image v-else :src="$utils.getImgUrl('myAvator.png')" width="60rpx" height="60rpx"></u-image>
					</view>
				</view>
				<view class="content_avatar_name">
					<u--text v-if="formData.username" color="#ffffff" :text="formData.username"></u--text>
					<u-button v-else type="text" @click="goNoLogin">欢迎登录</u-button>
				</view>
			</view>
	...
	methods: {
		getUserCenter() {
			...
			if (uid && token) {
				...
				this.$store.dispatch('user/getUserCenter', params).then(res => {
		...
		// 获取用户信息, 登录
		getUser(detail) {
			getUserFunc(detail, this.getUserCenter())
		},
		getUserError(error) {
			console.log(error);
		}

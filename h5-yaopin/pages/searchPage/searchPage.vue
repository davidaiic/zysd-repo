...
<template>
	<view class="mainWrap">
		...
		<view class="content" :style="{ height: scrollViewheight - 70 + 'px' }">
			<SearchList :searchModel.sync="searchModel" @back="goHome()" :defaultValue="defaultValue" @search=goToSearch />
...
export default {
	...
	onLoad(query) {
		this.getHeight();
		this.defaultValue = query.keyword || "";
		this.isCircle = !!query.isCircle
	...
	methods: {
		goToSearch() {
			...
			uni.navigateTo({
				url: `/pages/zzcx/ypcx?keyword=${this.searchModel}`,
			});
		},
		directSearch(item) {
			...
			uni.navigateTo({
				url: `/pages/zzcx/ypcx?keyword=${item}`,
			});
		},
		getHotWordList() {
			let params = {};
			this.$store.dispatch("home/getHomeHotWord", params).then((res) => {

<template>
    <view :id="keyId" class="select-content" :style="styleObj || ''">
        ...
        <view v-else class="select-box" @click.stop="selectSearch">
            <u--text lines="1" style="flex: 1" size="14" :color="!selectValue ? '#b2b2b2' : '#333'" :text="selectValue ||
                placeholder"> </u--text>
            <u-icon name="arrow-down" color="#999" size="14"></u-icon>
        </view>
        <view v-if="isShow" class="u-page">
            <u-loading-icon v-if="loading" :show="loading"></u-loading-icon>
            <template v-else>
                <u-list v-if="searchList && searchList.length" @scrolltolower="scrolltolower" height="500rpx">
...
export default {
    ...
    methods: {
        ...
        search() {
            ...
            this.$emit('search', (list) => {
                if (this.loading) {
                    ...
                    if (list && list.length) {
                        this.searchList = that.page > 1 ? this.searchList.concat(list) : list
                    } else {
                        this.moreStatus = 'nomore'
                    }
                }

                // this.isShow = !this.isCancle
                this.isMore = false
                this.loading = false
            }, params)
        },
        ...
        inputSearch: debounce(function (text) {
            ...
            this.isCancle = false
            if (this.remote) {
                this.$emit('update:currentValue', { [this.showKey]: text })
                this.$emit('complete')
            }
            if (!text) {
                return

				...
				<view class="hot_talk_content">
					<view class="hot_talk_content_item">
						...
						<view v-if="formData.pictures && formData.pictures.length" class="talk_content_img2">
							<u-image radius="8rpx" v-for="(img, imgkey) in formData.pictures" :height="imgWidth"
								...
								<view class="talk_content">
									<view class="talk_content_name">
										...
										<view class="talk_content_name_right" @click="toCommentLike(items)">
											...
											<u-image v-else class="talk_bottom_item_img"
												:src="$utils.getImgUrl('nolike.png')" width="32rpx"
...
export default {
	...
	methods: {
		...
		getCommentList() {
			...
			this.$store.dispatch('home/getHomeCommentInfo', params).then(res => {
		...
		shareComment(item) {
			...
			that.share = {
				title: '你收到来自朋友的一条评论',
				path: `/pages/tieDetail/tieDetail?commentId=${that.commentId}`
			}
		},
		success() {
			that.getCommentList()

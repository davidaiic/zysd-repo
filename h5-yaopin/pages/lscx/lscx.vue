<template>
	<view class="mainWrap">
		...
		<view class="content" :style="{ height: scrollViewheight + 'px' }">
			...
			<scroll-view class="lscx_items" @scrolltolower="loadMore" v-if="list.length > 0">
...
export default {
	...
	methods: {
		...
		getlscx() {
			...
			this.$store.dispatch('user/getUserQueryLog', params).then(res => {
				this.list = res.data.queryList

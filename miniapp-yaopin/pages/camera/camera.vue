<template>
	<view>
		<view class="cameraBg" v-if="loading">
			<camera v-if="type === 'pz'" device-position="back" flash="off" style="width: 100%; height: 100vh;position: relative;" mode="normal" @error="error">
			...
			<camera v-else device-position="back" :flash="flash" style="width: 100%; height: 100vh;position: relative;" mode="scanCode" @error="error" @scancode="scanCode">
				<cover-view class="camera-content">
					<cover-view class="content-head" :style="{ height: headerHeight }">
						<cover-image class="left" src="@/static/left-icon.png" @click="leftClick"></cover-image>
					...
					<cover-view class="bg-two">
						<cover-image src="@/static/camera/icon_example.png" :style="{ width: imgWidth, height: '120rpx', margin: '40rpx 116rpx' }"></cover-image>
				...
				<cover-view v-if="show" class="modal-view">
					<cover-view class="title-content">
						<cover-view class="title">{{ modal_info.modaltitle }}</cover-view>
						<u-icon name="close" color="#E0E0E0" size="20" @click="close"></u-icon>
					</cover-view>
					<cover-view class="slot-content">
						<cover-view class="word">{{ modal_info.content }}</cover-view>
						<cover-view class="word goods">{{ goodsName }}</cover-view>
					</cover-view>
					<cover-view class="foot">
						<cover-view class="foot-bt  foot-left" @click="close">
							<cover-view>{{ modal_info.cancleText }}</cover-view>
						</cover-view>
						<cover-view class="foot-bt  foot-right" @click="confirm">
							<cover-view>{{ modal_info.confirmText }}</cover-view>
						</cover-view>
					</cover-view>
				</cover-view>
			</camera>
			<cover-view class="scanBtn">
				<cover-image class="beatImg" src="@/static/camera/icon-album@2x.png" @click="chooseImage"></cover-image>
...
import { scanCodeReq } from '@/api/home.js';
// 移动动画
...
export default {
	data() {
		return {
			isScanSuccess: false, //是否成功
			loading: false,
			type: 'sys',
			scan_next_page_type:'',//扫码后跳转页面类型，scanZhenWei为真伪页面
			bgColor: '#080808',
			show: false,
			modal_info: {
	...
	onLoad(query) {
		...
		if(query.type == 'sys'){
			if(query.nextPageType == 'scanZhenWei'){
				this.scan_next_page_type = 'scanZhenWei'
			}
		}
	},
	mounted() {
	...
	methods: {
		...
		compress(tempFilePaths) {
			...
			uni.compressImage({
				...
				success: (imageRes) => {
					...
					let img_path = imageRes.tempFilePath
					
					// let img_path = tempFilePaths
					let nextFunction = that.type == 'sys' ? 
						that.scanCodeUploadFilePromise(img_path):
						that.uploadFilePromise(img_path)
						
					nextFunction.then(res => {
						uni.hideLoading();
						console.log(res)
						that.type == 'sys' ? that.scanCodeEnter(res.data): that.takePhoneEnter(res.data) 
					}).catch(res=>{
						console.log(res)
					});
				},
		...
		// 扫一扫后续处理
		scanCodeEnter(res_data){
			let that = this
			if (res_data.result === 1) {
				that.isScanSuccess = true;
				that.$set(that, 'modal_info', {
					modaltitle: '请确认是否为该商品',
					confirmText: '是',
					cancleText: '否，拍照查验',
					content: '您查真伪的是'
				});
				that.goodsName = res_data.goodsName;
				if(that.scan_next_page_type == 'scanZhenWei'){
					that.linkUrl = `/pages/result/gwwfw?goodsId=${res_data.goodsId}`
				}else{
					that.linkUrl = res_data.risk == 1 ? `/pages/result/smRisk?goodsId=${res_data.goodsId}` : `/pages/result/smNoRisk?goodsId=${res_data.goodsId}`;
				}
				that.show = true;
			} else {
		...
		// 扫一扫图片上传
		scanCodeUploadFilePromise(url) {
			let that = this;
			return new Promise((resolve, reject) => {
				uni.uploadFile({
					url: `${urlConfig}/query/scanQRCode`, // 仅为示例，非真实的接口地址
					filePath: url,
		...
		takePhoneEnter(res_data){
			...
			uni.navigateTo({
				url: `/pages/sbjg/pzjg/pzjg?result=${encodeURIComponent(JSON.stringify(res_data))}`
			});
		},
		// 拍照图片上传

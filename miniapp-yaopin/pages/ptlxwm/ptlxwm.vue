<template>
	<view class="mainWrap">
		<view class="contents">
			<u-navbar v-if="!isH5" title="联系我们" :fixed="false" @leftClick="leftClick"
				:titleStyle="{ color: '#FFF', fontSize: '18px' }"></u-navbar>
		</view>
		<scroll-view scroll-y class="content" :style="{ height: scrollViewheight + 'px' }">
			<!-- <u-navbar v-if="!isH5" title="联系我们" @leftClick="leftClick"  :titleStyle="{ color: '#FFF', fontSize: '18px' }"></u-navbar>
			<view class="content"> -->
			<view class="lxwm_content">
				...
				<u-gap height="20rpx" bgColor="#f8f8f8"></u-gap>
				<view class="lxwm_content_top" v-if="show_phone">
					<view class="lxwm_title">
						方式三：客服电话
					</view>
					<view class="lxwm_fac">
						<u-image :src="formData.icon" width="140rpx" height="140rpx"></u-image>
						<view class="lxwm_wxh">
							客服电话：{{ formData.mobile }}
						</view>
						<view class="">
							<u-button class="lxwm_copy" @click="tel(formData.mobile)">立即拨打电话</u-button>
						</view>
					</view>
				</view>
				<u-gap height="20rpx"></u-gap>
...
import {pluginContactReq} from '@/api/home.js'
export default {
	data() {
		return {
			show_phone:true,

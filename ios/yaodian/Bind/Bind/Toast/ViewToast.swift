...
extension UIView {
    ...
    */
    ...
    private func showToast(_ toast: UIView, duration: TimeInterval, point: CGPoint) {
        ...
        UIView.animate(withDuration: ToastManager.shared.style.fadeDuration, delay: 0.0, options: [.curveEaseIn, .allowUserInteraction], animations: {
            toast.alpha = 1.0
        }) { _ in
    ...
    private func hideToast(_ toast: UIView, fromTap: Bool) {
        ...
        UIView.animate(withDuration: ToastManager.shared.style.fadeDuration, delay: 0.0, options: [.curveEaseOut, .beginFromCurrentState], animations: {
            toast.alpha = 0.0
        }) { _ in
            toast.removeFromSuperview()
...
public enum ToastPosition {
    ...
    case center
    case bottom
    case centerX(customY: CGFloat)
    case custom(centerPoint: CGPoint)
    
    fileprivate func centerPoint(forToast toast: UIView, inSuperview superview: UIView) -> CGPoint {
        let topPadding: CGFloat = ToastManager.shared.style.verticalPadding + superview._safeAreaInsets.top
        let bottomPadding: CGFloat = ToastManager.shared.style.verticalPadding + superview._safeAreaInsets.bottom
        switch self {
        case .top:
        ...
        case .centerX(let customY):
            return CGPoint(x: superview.bounds.size.width / 2.0, y: customY)

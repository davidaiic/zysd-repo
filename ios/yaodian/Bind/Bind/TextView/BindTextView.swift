...
public protocol BindTextViewDelegate: UITextViewDelegate {
...
@IBDesignable @objc
open class BindTextView: UITextView {
    open weak var growingDelegate: BindTextViewDelegate?
    ...
    private func associateConstraints() {
        ...
        // height,from: https://github.com/legranddamien/MBAutoBindTextView
    ...
    override open func layoutSubviews() {
        ...
        if height != heightConstraint!.constant {
            ...
            if let delegate = growingDelegate as? BindTextViewDelegate {
    ...
    @objc func textDidEndEditing(notification: Notification) {
        if let sender = notification.object as? BindTextView, sender == self {
    ...
    @objc func textDidChange(notification: Notification) {
        if let sender = notification.object as? BindTextView, sender == self {
            if maxLength > 0 && text.count > maxLength {
                ...
                growingDelegate?.textBeyondMaxCount(self)
            }
            
            setNeedsDisplay()
            growingDelegate?.textDidChange(self)
        }
        
    }
}

extension BindTextView: UITextPasteDelegate {
    @available(iOS 11.0, *)
